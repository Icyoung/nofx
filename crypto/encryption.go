...
const (
	rsaKeySize        = 4096
	rsaPrivateKeyFile = "secrets/rsa_key"
	rsaPublicKeyFile  = "secrets/rsa_key.pub"
...
func (em *EncryptionManager) loadOrGenerateRSAKeyPair() error {
	...
	if _, err := os.Stat(rsaPrivateKeyFile); err != nil {
		return fmt.Errorf("RSA private key file not found at %s: %w (keys should be generated by start script)", rsaPrivateKeyFile, err)
	}
...
func (em *EncryptionManager) loadRSAKeyPair() error {
	...
	// 使用统一的私钥解析逻辑（支持PKCS#1和PKCS#8格式）
	privateKey, err := ParseRSAPrivateKeyFromPEM(privateKeyPEM)
	if err != nil {
		return fmt.Errorf("無效的私鑰 PEM 格式: %w", err)
	}
